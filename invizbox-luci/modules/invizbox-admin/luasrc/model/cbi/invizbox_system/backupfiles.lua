...
if luci.http.formvalue("cbid.luci.1._list") then
	luci.http.redirect(luci.dispatcher.build_url("invizbox/flashops/backupfiles") .. "?display=list")
elseif luci.http.formvalue("cbid.luci.1._edit") then
	luci.http.redirect(luci.dispatcher.build_url("invizbox/flashops/backupfiles") .. "?display=edit")
	return
end

m = SimpleForm("luci", translate("Backup file list"))
m:append(Template("invizbox_system/backupfiles"))
