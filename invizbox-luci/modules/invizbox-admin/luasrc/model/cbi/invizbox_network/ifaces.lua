...
if m:formvalue("cbid.network.%s._switch" % net:name()) then
	...
	if proto then
		...
		-- reload page
		luci.http.redirect(luci.dispatcher.build_url("invizbox/network/network", arg[1]))
		return
	end
end
...
local function set_status()
	...
	if not net:is_floating() and net:is_empty() then
		...
		st.template = "invizbox_network/iface_status"
		st.network  = arg[1]
		st.value    = nil
	end
end
...
if not net:is_installed() then
	...
	function p_install.write()
		return luci.http.redirect(
			luci.dispatcher.build_url("invizbox/packages") ..
...
local form, ferr = loadfile(
	ut.libpath() .. "/model/cbi/invizbox_network/proto_%s.lua" % net:proto()
)
