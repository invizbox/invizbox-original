...
function m.parse(map)
	if m:formvalue("cbid.wireless.%s.__toggle" % wdev:name()) then
		...
		luci.http.redirect(luci.dispatcher.build_url("invizbox/network/wireless", arg[1]))
		return
	end
	Map.parse(map)
end
...
back.value = ""
back.titleref = luci.dispatcher.build_url("invizbox", "network", "wireless")
]]

st = s:taboption("general", DummyValue, "__status", translate("Status"))
st.template = "invizbox_network/wifi_status"
st.ifname   = arg[1]
